...
export const useDataTableState = create<DataTableState>()(
  devtools((set) => ({
    ...
    columns: [],
    setColumns: (columns) => set((state: DataTableState) => ({ columns })),
    fetchedData: (data: DataItem[]) => {
      set((state: DataTableState) => ({ fetchedData: data }));
    },

    setCurrentPage: (page) => {
      set((state: DataTableState) => ({ currentPage: page }));
    },
    setPageSize: (size) => set((state: DataTableState) => ({ pageSize: size })),
    calculateTotalPages: (totalItems, pageSize) => {
      return Math.ceil(totalItems / pageSize);
    },

    setSortBy: (column) => set((state: DataTableState) => ({ sortBy: column })),
    setSortDirection: (direction) => {
      set((state: DataTableState) => ({ sortDirection: direction }));
    },
    resetSort: () => {
      set((state: DataTableState) => ({ sortBy: "", sortDirection: "ASC" }));
    },
    ...
    resetFilters: () => {
      set((state: DataTableState) => ({ filters: [] }));
    },
